...
export async function POST(request: NextRequest) {
  ...
  if (type === 'student-register') {
    const existing = await db.prepare('SELECT * FROM students WHERE username = ?').get(username);
    if (existing) {
      return NextResponse.json({ error: 'Username already exists' }, { status: 400 });
    }
    const hash = bcrypt.hashSync(password, 10);
    const now = new Date().toISOString();
    const info = await db.prepare('INSERT INTO students (lastname, firstname, middlename, username, password, year_section, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(lastname, firstname, middlename || '', username, hash, '2-A', now, now);
    return NextResponse.json({ success: true, studentId: info.lastInsertRowid });
  }

  if (type === 'student-login') {
    const user = await db.prepare('SELECT * FROM students WHERE username = ?').get(username) as any;
  ...
  if (type === 'teacher-login') {
    const user = await db.prepare('SELECT * FROM teachers WHERE username = ?').get(username) as any;
    if (!user || !bcrypt.compareSync(password, user.password)) {
