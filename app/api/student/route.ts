...
export async function POST(request: NextRequest) {
  ...
  if (body.type === 'update-section') {
    db.prepare('UPDATE students SET year_section = ?, updated_at = ? WHERE id = ?').run(body.year_section, new Date().toISOString(), body.studentId);
  ...
  if (body.type === 'scan-qr') {
    ...
    const session = db.prepare('SELECT * FROM attendance_sessions WHERE session_id = ?').get(parsed.session_id) as any;
    if (!session) {
      return NextResponse.json({ error: 'Invalid QR code' }, { status: 400 });
    }
    ...
    const existing = db.prepare('SELECT * FROM attendance_records WHERE session_id = ? AND student_id = ?').get(parsed.session_id, studentId) as any;
    if (existing) {
    ...
    db.prepare('INSERT INTO attendance_records (session_id, student_id, subject_id, date, time, status, year_section, student_name, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(parsed.session_id, studentId, session.subject_id, date, time, 'Present', year_section, studentName, now.toISOString());
